...
class HouseRulesController extends GetxController {
  static HouseRulesController get instance => Get.find();

  final RxList<HouseRule> rules = <HouseRule>[].obs;
  final RxBool isLoading = false.obs;
  final RxString error = ''.obs;
  final RxInt retryCount = 0.obs;
  final int maxRetries = 2;

  Future<void> fetchRules({String? roomId, bool isRetry = false}) async {
    try {
      ...
      final endpoint = roomId != null 
        ? '${Config.houseRulesEndpointbyId}'
        : '${Config.houseRulesEndpoint}';
  ...
  void _handleResponse(http.Response response) {
    if (response.statusCode == 200) {
      ...
      rules.assignAll(data.map((json) => HouseRule.fromJson(json)));
    } else if (response.statusCode == 401 && retryCount.value < maxRetries) {
      _handleUnauthorizedError();
    } else {
      error('Failed to load rules: ${response.statusCode}');
      ALoaders.errorSnackBar(title: 'Error', message: 'Failed to load rules');
    }
  }
  ...
  void _handleGenericError(dynamic e) {
    error(e.toString());
    ALoaders.errorSnackBar(title: 'Error', message: 'Failed to fetch rules');
  }
}